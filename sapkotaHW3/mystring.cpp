...
#include <cctype>		// Provides stream tools
#include <cassert>		// Provides assert()
#include <cstring>		// Provides strcpy(), strcat(), strlen(), strcmp()
...
void my_string::insertNewline(size_t n)
    {
        size_t totalLength;
        totalLength = current_length +2;
        reserve (totalLength);
    ...
    // this is added new function add hypen and newline charector as nessessary
    
    void my_string::insertHypenNewline(size_t n)
    {
        int nextPosition =n;
        
        while (n < getCurrentLength())
        {
            // insert hypen and newline as needed
            if ((sequence[n-1] != ' ') && (sequence[n] != ' '))
            {
                size_t totalLength;
                totalLength = current_length+3;
                reserve(totalLength);
                current_length = totalLength-1;
            ...
            // Insert only newline at column position
            {
                size_t totalLength;
                totalLength = current_length+2;
                reserve(totalLength);
